...
export default async function handler(req, res) {
    if (req.method !== 'POST') return res.status(405).json({ success: false, error: '非法请求：黑盒拒绝访问' });
    ...
    try {
        ...
        // 为了节省字数，这里使用简化的判定词。如果需要，你可以在部署后把自己的几千字文案贴回 detailedDesc 中。
        ...
        const forcedQuestions = [
            ...
            { id: 203, sandbox:'AN', qtype:'Q1', opts:['A. 一个稳定的方向和身份锚点——让我知道自己一直在积累','B. 清晰的规划和阶段目标——每一步都踏在预定的轨道上','C. 不断有新的机会和可能性打开——让我看到更大的版图','D. 持续突破瓶颈的快感——冲不过去的东西让我兴奋','E. 跳出来重新定义"成长"本身——不被旧路径锁死'] },
            ...
            { id: 206, sandbox:'RN', qtype:'Q1', opts:['A. 对方稳定的回应和陪伴——让我感到有安全底座','B. 清晰的边界和分工——让我知道什么是我的、什么是对方的','C. 关系能带来更多的连接和资源——让我的世界变得更大','D. 能推动关系变化的冲动——冲突和张力让我感觉自己还活着','E. 跳出情绪看清关系的底层模式——不被绑定'] },
            ...
            { id: 208, sandbox:'RN', qtype:'Q3', opts:['A. 让对方看到我真实的需要——我怕被拒绝','B. 打破沉默和僵局——我不确定冲突会不会让关系更糟','C. 在关系混乱的时候提出明确的规则——我怕显得太冷','D. 信任他人并在适当的时候依赖他们——我怕暴露弱点','E. 主动结束一段已经不再滋养我的关系——我怕失去'] },
            { id: 209, sandbox:'IN', qtype:'Q1', opts:['A. 一个稳定的位置和归属感——让我在群体里有落点','B. 清楚的规则和角色——让我知道怎样不乱位','C. 足够大的活动半径和辐射力——让更多人看到我','D. 能表达、竞争和争取位置的冲动——让我不被动吞没','E. 跳出评价看透群体结构——不被舆论牵着走'] },
            ...
            { id: 211, sandbox:'IN', qtype:'Q3', opts:['A. 被孤立——没有一个人站在我这边','B. 被评价体系裹挟——失去了自己的判断标准','C. 位置被取代——我在群体里的位置和话语权动摇','D. 被看到真实的自己——他们可能会发现我并不如表面那样','E. 冲突升级——我本意只是想表达不同意见'] },
            { id: 212, sandbox:'MN', qtype:'Q1', opts:['A. 一个能长期确认自我认同的信念锚点——让我不漂移','B. 一套清晰自洽的信念结构——让我的目标不散乱','C. 让我的观念影响更多人——进入更大的现实空间','D. 不断质疑和打破旧信念的冲动——让意义不僵化','E. 从更高层重构意义的能力——让我获得精神自由'] },
            ...
            { id: 215, sandbox:'FN', qtype:'Q1', opts:['A. 固定的习惯和可依赖的模板——让系统不因变化而失灵','B. 清晰的流程和反馈闭环——让系统低损耗运行','C. 可复制放大的结构——让系统规模越来越大','D. 敢于打破旧流程的动力——不让系统被低效拖死','E. 抽离出来理解底层逻辑的能力——获得更高维控制'] },
            { id: 216, sandbox:'FN', qtype:'Q2', opts:['A. 外部系统让我失望过——之后我自己给自己搭系统','B. 我遵守好的系统并从中受益——更信任清晰的外部规则','C. 外部环境太乱了——我不得不建一个自己能控制的空间','D. 我试着建立过自己的系统，但很难坚持下来','E. 我没有特别关注过外部系统的规则或流程'] },
            { id: 217, sandbox:'FN', qtype:'Q3', opts:['A. 系统失效——我依赖的秩序崩塌了','B. 规则太密把自己也困住了','C. 规模超出承载——扩张失控','D. 系统变旧后我没有推倒重来的勇气','E. 沉浸在系统设计里，忽略了现实的执行和落地'] }
        ];
        ...
        for(let s of sandboxes) matrixHtml += `<th style="cursor:pointer; text-decoration:underline dotted;" onclick="alert(explanations.sandboxes['${s}'])">${s}</th>`;
        matrixHtml += `<th>总分</th> </thead><tbody>`;
        for(let e of engines) {
            matrixHtml += `<tr><td style="font-weight:600"><span style="cursor:pointer; text-decoration:underline dotted;" onclick="alert(explanations.engines['${engineFull[e]}'])">${engineFull[e]}</span></td>`;
            for(let s of sandboxes) {
                let sc = engineScores[e][s] || 0;
                let st = seedStatus[e][s];
                let colorClass = st==='繁茂'?'status-bloom':(st==='生长'?'status-grow':(st==='卷缩'?'status-shrink':'status-buried'));
                matrixHtml += `<td class="${colorClass}">${sc}<br><span style="cursor:pointer; border-bottom:1px dashed #666;" onclick="alert(explanations.energy['${st}'])">${st}</span></td>`;
            }
        ...
        let forcedRecords = `<h3>四、🔍 9号追选题作答记录</h3><table><thead><tr><th>沙盒</th><th>作答</th></tr></thead><tbody>`;
        for(let s of sandboxes) {
            let q3 = forcedQuestions.find(f => f.sandbox===s && f.qtype==='Q3');
            forcedRecords += `<tr><td>${sandboxFull[s]}</td><td>已加密收录</td></tr>`;
        }
        forcedRecords += `</tbody></table>`;

        let chainHtml = `<h3>五、⛓️ 因果链与生态分析</h3><p>系统已记录生克矩阵，触发因果链：旺克衰机制已就绪。</p>`;

        let finalReportHtml = matrixHtml + engineBodyHtml + sandboxBodyHtml + forcedRecords + chainHtml;
        ...
        return res.status(200).json({ success: true, recordId: data && data.length > 0 ? data[0].id : null, reportHtml: finalReportHtml });

    } catch (error) {
        console.error('黑盒错误:', error);
